...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Usar callbackUrl e redirecionamento direto para o Google
      const result = await signIn("google", {
        callbackUrl: "/dashboard",
        redirect: false,
      })

      // Se não houver redirecionamento automático, verificar erros
      if (result?.error) {
        setLoginError(`Erro ao fazer login com Google: ${result.error}`)
        setGoogleLoading(false)
      } else if (result?.url) {
        // Redirecionar manualmente para a URL do Google Auth
        window.location.href = result.url
      }
    } catch (error) {
      console.error("Erro ao fazer login com Google:", error)
      setLoginError(`Erro ao fazer login com Google: ${error instanceof Error ? error.message : "Erro desconhecido"}`)
      setGoogleLoading(false)
    }
  }
