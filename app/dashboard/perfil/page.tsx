...
async function PerfilLojaContent() {
  try {
          ...
          <Card className="h-full">
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-center">
                <div className="bg-white p-2 rounded-lg w-full max-w-[180px] mx-auto">
                  <QRCodeSVG
                    value={vitrineUrl}
                    size={160}
                    level="M"
              ...
              <div className="text-xs text-center text-muted-foreground mb-2 mt-1">{vitrineUrl}</div>
              <div className="space-y-2">
                <Button asChild variant="outline" className="w-full" size="sm">
                  <Link href={`/dashboard/perfil/vitrine?tab=qrcode&id=${loja._id}`}>
                    <Printer className="mr-2 h-4 w-4" />
                    Imprimir Código
                  </Link>
                </Button>
                <Button asChild variant="outline" className="w-full" size="sm">
                  <Link href={`/dashboard/perfil/vitrine?id=${loja._id}`}>
                    <Edit className="mr-2 h-4 w-4" />
                    Gerenciar Vitrine
                  </Link>
                </Button>
    ...
    console.error("Erro ao renderizar perfil da loja:", error)
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle>Erro ao carregar perfil</CardTitle>
          <CardDescription>Ocorreu um erro ao carregar os dados da loja.</CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
            <Link href="/dashboard/perfil/editar">
