...
export function WhatsappIntegracao() {
  ...
  const chatMessageSocketRef = useRef<Socket | null>(null); // Ref for the chat message WebSocket

  // Session ID is now generated by the backend
  ...
  const resetToInitialState = () => {
    ...
    if (chatMessageSocketRef.current) {
      console.log('Resetting: Disconnecting chat message WebSocket.');
      chatMessageSocketRef.current.disconnect();
      chatMessageSocketRef.current = null;
    }
  };
  ...
  const handleStartConfiguration = async () => {
    ...
    // TODO: Replace these with actual logic to get lojaId and userId
    // e.g., from context, props, or session.
    const lojaId = "REPLACE_WITH_ACTUAL_LOJA_ID";
    const userId = "REPLACE_WITH_ACTUAL_USER_ID";

    if (lojaId === "REPLACE_WITH_ACTUAL_LOJA_ID" || userId === "REPLACE_WITH_ACTUAL_USER_ID") {
      toast({
        title: "Erro de Configuração",
        description: "Loja ID ou User ID não estão configurados no frontend.",
        variant: "destructive",
      });
      setIsProcessing(false);
      return;
    ...
    try {
      const response = await fetch("/api/integracoes/whatsapp/start", {
        ...
        body: JSON.stringify({ lojaId, userId }),
      });
      ...
      const backendSessionId = responseData.sessionId;

      if (!backendSessionId) {
        throw new Error("Session ID não recebido do backend.");
      }
      
      setSessionId(backendSessionId); // Set sessionId from backend response
      ...
      setStep(2);
      await requestNewQrCode(backendSessionId); // Use sessionId from backend
  ...
  useEffect(() => {
    if (step === 2 && isQrActive && connectionStatus === "connecting" && sessionId && !socketRef.current) {
      // Conecta ao servidor Socket.IO local
      // A URL base é a origem atual da janela (window.location.origin)
      // O 'path' deve corresponder ao configurado no backend Socket.IO (app/api/integracoes/whatsapp/socket/route.ts)
      const socketIoPath = "/api/integracoes/whatsapp/socket";

      // Para conexões WebSocket, o io() por padrão tentará conectar à mesma origem da página.
      // Se o servidor Socket.IO estiver no mesmo host e porta, apenas o path é necessário.
      // Se estiver em um domínio/porta diferente, a URL completa seria `io("http://localhost:3000", { path: socketIoPath, ... })`
      // ou `io(window.location.origin, { path: socketIoPath, ... })`
      
      const newSocket = io({ // Conecta à origem atual
        path: socketIoPath,
        query: { sessionId },
  ...
  // useEffect for chat message WebSocket
  useEffect(() => {
    // This effect manages the WebSocket connection for real-time chat messages.
    if (connectionStatus === "connected") {
      if (chatMessageSocketRef.current && chatMessageSocketRef.current.connected) {
        // Already connected and connection is active
        return;
      }

      console.log('Attempting to connect to WebSocket for chat messages: wss://fletoapi.robotizze.us');
      const newChatMessageSocket = io('wss://fletoapi.robotizze.us', {
        transports: ['websocket']
        // Add any necessary auth query parameters here if required by the server
        // query: { token: 'your_auth_token', sessionId: sessionId }
      });
      chatMessageSocketRef.current = newChatMessageSocket;

      newChatMessageSocket.on('connect', () => {
        console.log('Conectado ao WebSocket da Evolution API'); // As per prompt for wss://fletoapi.robotizze.us
      });

      newChatMessageSocket.on('message_upsert', (data: any) => { // Added type any for data
        console.log('Evento message_upsert recebido:', data);
        // TODO: Add logic here to handle the incoming message data
        // This could involve updating the UI, playing sounds, etc.
      });

      newChatMessageSocket.on('disconnect', (reason: Socket.DisconnectReason) => { // Added type for reason
        console.log('Desconectado do WebSocket da Evolution API:', reason); // As per prompt for wss://fletoapi.robotizze.us
        // Clean up ref on explicit disconnect
        if (chatMessageSocketRef.current === newChatMessageSocket) {
             chatMessageSocketRef.current = null;
        }
      });

      newChatMessageSocket.on('connect_error', (error: Error) => { // Added type for error
        console.error('Chat Message WebSocket connection error (wss://fletoapi.robotizze.us):', error);
        if (chatMessageSocketRef.current === newChatMessageSocket) {
             chatMessageSocketRef.current = null; // Clean up ref on connection error
        }
      });

      return () => {
        console.log('Cleaning up chat message WebSocket connection (wss://fletoapi.robotizze.us).');
        newChatMessageSocket.disconnect();
        if (chatMessageSocketRef.current === newChatMessageSocket) {
            chatMessageSocketRef.current = null;
        }
      };
    } else {
      // If the primary connection is not 'connected', ensure chat message socket is disconnected.
      if (chatMessageSocketRef.current) {
        console.log('Disconnecting chat message WebSocket (wss://fletoapi.robotizze.us) due to primary connection status change.');
        chatMessageSocketRef.current.disconnect();
        chatMessageSocketRef.current = null;
      }
    }
  }, [connectionStatus, sessionId]); // Added sessionId as a dependency in case it's needed for auth/query
