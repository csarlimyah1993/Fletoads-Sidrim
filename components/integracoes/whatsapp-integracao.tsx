    ...
    try {
      ...
      setIntegracoes((prevIntegracoes) =>
        prevIntegracoes.map((integracao) =>
          integracao._id === id ? { ...integracao, status: data.status } : integracao,
        ),
      )
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Conecte seu WhatsApp para enviar mensagens através da Evolution API</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          integracoes.map((integracao) => (
            <Card key={integracao._id}>
              <CardHeader>
                ...
                <CardDescription>
                  {integracao.telefone ? `Conectado ao número ${integracao.telefone}` : "Nenhum telefone conectado"}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
              ...
              <CardFooter className="flex justify-between">
                ...
                <div className="flex space-x-2">
                  {integracao.status === "conectado" && (
                    <Button variant="outline" size="sm" onClick={() => desconectarWhatsapp(integracao._id)}>
                      <LogOut className="mr-2 h-4 w-4" />
                      Desconectar
                    </Button>
                  )}
                  <Button variant="destructive" size="sm" onClick={() => excluirIntegracao(integracao._id)}>
            ...
            {qrCodeLoading ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="mt-4 text-sm text-muted-foreground">
                  Abra o WhatsApp no seu celular, vá em Configurações &gt; Aparelhos conectados &gt; Conectar um
                  aparelho
                </p>
              </div>
            ) : (
